RSpec.describe CodeOwnership::Private::Validations::FilesHaveOwners do
  describe 'CodeOwnership.validate!' do
    let(:codeowners_validation) { Private::Validations::GithubCodeownersUpToDate }

    context 'when input files are not part of configured owned_globs' do
      before do
        write_file('Gemfile', '')
    ...
    context 'when a file in owned_globs does not have an owner' do
      before do
      ...
      context 'when the file is not in unowned_globs' do
        before do
          write_configuration
        end
        ...
        context 'when the input files do not include the file missing ownership' do
      ...
      context 'when that file is in unowned_globs' do
        before do
    ...
    context 'when many files in owned_globs do not have an owner' do
      before do
        write_configuration
