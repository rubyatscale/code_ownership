    ...
    describe 'CodeOwnership.validate!' do
      context 'a file in owned_globs has ownership defined in multiple ways' do
        before do
          ...
          write_file('frontend/javascripts/packages/my_package/.codeowners', <<~CONTENTS)
            Bar
          CONTENTS
        ...
        it 'lets the user know that each file can only have ownership defined in one way' do
          ...
          expect { CodeOwnership.validate! }.to raise_error do |e|
            ...
            expect(e.message).to eq <<~EXPECTED.chomp
              ...
              - frontend/javascripts/packages/my_package/owned_file.jsx (Annotations at the top of file, Team-specific owned globs, Owner in .codeowners, Owner metadata key in package.json)
              - frontend/javascripts/packages/my_package/package.json (Team-specific owned globs, Owner in .codeowners, Owner metadata key in package.json)
