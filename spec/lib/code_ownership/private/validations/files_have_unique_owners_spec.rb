RSpec.describe CodeOwnership::Private::Validations::FilesHaveUniqueOwners do
  describe 'CodeOwnership.validate!' do
    context 'when a file in owned_globs has ownership defined in multiple ways' do
      before do
        write_configuration
      ...
      it 'lets the user know that each file can only have ownership defined in one way' do
        expect(CodeOwnership.for_file('app/missing_ownership.rb')).to be_nil
